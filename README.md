...
│   ├── local-business-ja.json    # LocalBusiness スキーマ（日本住所形式・食べログ等 sameAs 対応）
...
### 引用性スコアリング
各コンテンツブロックを「AI に引用されやすいか」という観点で 0〜100 点で評価します。引用されやすいパッセージには「134〜167語程度」「前後の文脈なしで意味が伝わる」「具体的な数値や事実が含まれる」「質問に直接答える形になっている」といった共通点があります。**日本語テキストにも対応**しており、文字数ベースの長さ評価・日本語の定義表現（〜とは、〜のことです）・文構造の検出など、日本語コンテンツに最適化したスコアリングを行います。

### AI クローラー分析
GPTBot・ClaudeBot・PerplexityBot など 14 種類以上の AI クローラーが robots.txt でブロックされていないか確認し、推奨設定を提示します。

### ブランド言及スキャン
バックリンクよりもブランド言及のほうが AI 可視性との相関が 3 倍強いことがわかっています（Ahrefs 調査）。YouTube・Reddit・Wikipedia・LinkedIn などの国際プラットフォームに加え、**X（Twitter）・Yahoo!知恵袋・食べログ・価格.com・@cosme・note・Qiita・Zenn・はてなブックマーク・Google ビジネスプロフィール**など国内 10 以上のプラットフォームも調査対象です。

### プラットフォーム別最適化
同じ検索クエリに対して ChatGPT と Google AI Overviews の両方に引用されているサイトは全体の 11% しかありません。Google AIO・ChatGPT・Perplexity・Gemini・Bing Copilot に加え、**Yahoo! Japan**（知恵袋・Yahoo!ロコ・日本語ソーシャルシグナルを含む）にも対応した改善策を提案します。
...
日本のユーザーに最適化した機能を標準搭載しています。

| 機能 | 詳細 |
|------|------|
| X（Twitter）スキャン | 日本最大の SNS でのブランド言及状況を確認 |
| Yahoo!知恵袋スキャン | 日本版 Q&A プラットフォームでの質問・言及を調査 |
| 国内レビューサイトスキャン | 食べログ・価格.com・@cosme・はてなブックマーク等に対応 |
| テック媒体スキャン | note・Qiita・Zenn でのブランド・製品言及を確認 |
| Google ビジネスプロフィール確認 | GBP の登録状況と最適化状態をチェック |
| Yahoo! Japan プラットフォーム評価 | 知恵袋・Yahoo!ロコ・日本語ソーシャルシグナルを含む評価 |
| 日本語引用性スコアリング | 日本語テキストを文字数ベースで正確に評価 |
| 特定商取引法チェック | EC サイト必須の特商法ページの有無と記載内容を確認 |
| hreflang 日本向けガイダンス | `ja` と `ja-JP` の使い分けを含む国際化タグの検証 |
| 日本住所スキーマ | 都道府県・区市町村・丁目番地形式の JSON-LD テンプレート |
